...
class ArrayInfo {
    ...
    std::string dim_bitcount_stmt(int dim) const {
        assert(dim >= 0);
        assert(dim < ndim());
        return dim_bitcount_variable(dim) + " = num_bits((" + dim_max_string(dim) +
               ") - (" + dim_min_string(dim) + ") + 1);";
    }
...
class ArrayPack {
    ...
    ArrayPack(isl_union_map* R, isl_union_map* W) {
        ...
        for(const auto& it : builders) {
            ...
            info_.push_back(ArrayInfo{it.first, sec.id, sec.ndim, num_id_bits,
